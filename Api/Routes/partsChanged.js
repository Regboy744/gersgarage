...
router.post("/create/:register", verify, async (req, res) => {
  ...
  if (!vehicle) return res.status(400).send("This vehicle does not existe in the data base");
  ...
  if (counter == null) return res.status(400).send("There is no Service Ordem opened to this Vehicle");
